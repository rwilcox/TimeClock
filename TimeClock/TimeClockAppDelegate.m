...
@synthesize timeClock = _timeClock;
...
{
    if(_timeClock == nil) {
        _timeClock = [[TimeClock alloc] initWithManagedObjectModel:self.managedObjectModel managedObjectContext:self.managedObjectContext];
    }
    return _timeClock;
}
...
- (void)printAction:(id)sender
{
    ...
    for (Project *project in projectsArrayController.selectedObjects) {
        [self.timeClock printProject:project asHTMLtoMutableString:string];
    }
    NSLog(@"%@", string);
